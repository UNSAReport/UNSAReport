...
4. Run `node prepare.js <lab-dir>` to compile the report and create the submission bundle.
...
```bash
node capture.js l1/img/lab/session "typst compile l1/report.typ l1/report.pdf"
```
...
`prepare.js` writes the compiled PDF and source archive into the `submission/` folder inside the lab directory (e.g., `l1/submission/`).
